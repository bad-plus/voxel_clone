...
ChunkInfo* World::createChunk(int x, int z) {
    ...
    pushGeneriationQueue(chunk_info);
    return chunk_info;
}
...
void World::pushGeneriationQueue(ChunkInfo* chunk_info) {
...
void World::pushToChunks(ChunkInfo* chunk_info) {
    ...
    const long long chunk_index = pack_chunk_coords(chunk_info->x, chunk_info->z);
...
void World::processGenerationQueue() {
    ...
    ChunkInfo* x_m = getChunkProtected(x - 1, z);
    if(x_m != nullptr) {
    ...
    ChunkInfo* z_m = getChunkProtected(x, z - 1);
    if(z_m != nullptr) {
    ...
    ChunkInfo* x_p = getChunkProtected(x + 1, z);
    if(x_p != nullptr) {
    ...
    ChunkInfo* z_p = getChunkProtected(x, z + 1);
    if(z_p != nullptr) {
